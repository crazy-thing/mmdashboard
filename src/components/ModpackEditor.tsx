...
import { editModpack, uploadFile } from '../util/api'
...
const ModpackEditor = ({ modpack, fetchModpacks, setShowCreateModpack }: ModpackEditorProps) => {
    ...
    const handleUploadFile = async (file: File, filename: 'backgroundFile' | 'thumbnailFile') => {
        await uploadFile(
            file,
            `${import.meta.env.VITE_IP}`,
            localStorage.getItem('apiKey') as string,
    ...
    const handleBackgroundChange = async (event: any) => {
        ...
        handleUploadFile(newFile, 'backgroundFile');
        await handleSave("background", newFile.name);
    };

    const handleThumbnailChange = async (event: any) => {
        const file = event.target.files[0];
        const newFile = new File([file], `${file.name}-${Date.now()}`);
        handleUploadFile(newFile, 'thumbnailFile');
        await handleSave("thumbnail", newFile.name);
    };
